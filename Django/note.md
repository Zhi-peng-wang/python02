...
# 2.其它简单视图
- django.http给我们提供类很多和HttpResponse类似的简单的视图，通过查看django.http代码我们知道
- 此类视图使用方法基本类似，可以通过return语句转换直接反馈返回给浏览器
- Http404为Execption的子类，所以需要使用raise
# 3.HttpResponse详解
- 方法
    - init：使用页面内容实例化HttpResponse对象
    - write(content):以文件的方式写入
    - flush():以文件的方式输出缓存区
    - set_cookie(key, value="", max_age=None, expires=None):设置Coookie
        - key,value都是字符串类型
